...
Live crypto (btcusd, ethusd), equity (aapl), and 60-second TWAP were available. The TWAP full_accuracy_value is a decimal string, not an E18 integer. The 30-second window and btcusdt returned empty barriers. No dropped envelope or auth_required rejection could be elicited in staging shadow authentication mode. Deterministic malformed/precision cases are explicitly synthetic tests, not captured frames. These captures do not establish live coverage for the empty feeds or credential enforcement.
